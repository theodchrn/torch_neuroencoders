...
def barplot_linError(
    ...
    decoderForPD = ["ANN"] * lErrorNN_mean.shape[0] * len(timeWindows) + [
        "Bayes"
    ] * lErrorNN_mean.shape[0] * len(timeWindows)
...
def barplot_euclError(
    ...
    decoderForPD = ["ANN"] * errorNN_mean.shape[0] * len(timeWindows) + [
        "Bayes"
    ] * errorNN_mean.shape[0] * len(timeWindows)
    datToPlot = pd.DataFrame(
        {
            "eucl. error (cm)": data,
            "timeWindow (ms)": timeWindowsForDF,
...
def barplot_linError_mouse_by_mouse(
    ...
    for iWindow in range(len(timeWindows)):
        data = np.round(
            np.vstack(
                (lErrorNN_mean[:, iWindow], lErrorBayes_mean[:, iWindow])
            ).flatten(),
            2,
        )
        mouseNamesForDF = mouseNames * 2
        decoderForPD = ["ANN"] * lErrorNN_mean.shape[0] + [
            "Bayes"
        ] * lErrorBayes_mean.shape[0]
        dataByWindow.append(
            pd.DataFrame(
                {
                    "linearError": data,
...
def barplot_euclError_mouse_by_mouse(
    errorNN_mean,
    errorBayes_mean,
    timeWindows=[36, 108, 252, 504],
    mouseNames=["994", "1199_1", "1199_2", "1223"],
    ...
    for iWindow in range(len(timeWindows)):
        data = np.round(
            np.vstack(
                (errorNN_mean[:, iWindow], errorBayes_mean[:, iWindow])
            ).flatten(),
            2,
        )
        mouseNamesForDF = mouseNames * 2
        decoderForPD = ["ANN"] * errorNN_mean.shape[0] + [
            "Bayes"
        ] * errorBayes_mean.shape[0]
        dataByWindow.append(
            pd.DataFrame(
                {
                    "eucl. error (cm)": data,
...
def plot_euclError_mouse_by_mouse(
    errorNN_mean,
    errorBayes_mean,
    errorNN_err,
    errorBayes_err,
    ...
    for iWindow in range(len(timeWindows)):
        ...
        ax.errorbar(
            ...
            yerr=errorNN_err[:, iWindow],
            fmt="-o",
            color=colorsForSNS[0],
            label="ANN",
        )
        ax.errorbar(
            mouseNames,
            errorBayes_mean[:, iWindow],
            yerr=errorBayes_err[:, iWindow],
            fmt="-o",
            color=colorsForSNS[1],
            label="Bayes",
        )
        ...
        if dirSave is not None:
            fig.savefig(
                os.path.join(
                    dirSave, f"errorPlotMBM{suffix}_{timeWindows[iWindow]}.png"
                )
            )
            fig.savefig(
                os.path.join(
                    dirSave, f"errorPlotMBM{suffix}_{timeWindows[iWindow]}.svg"
                )
            )


def boxplot_euclDist_decoders(
    distMean, timeWindows=[36, 108, 252, 504], dirSave=None, suffix=""
):
    # TODO: add significance test?
...
def fig_eucl_error_filtered(
    ...
    timeWindowsForDF = timeWindows * errorNN_mean.shape[0] * 2
    decoderForPD = ["Full"] * errorNN_mean.shape[0] * len(timeWindows) + [
        "30% best"
    ] * errorNN_mean.shape[0] * len(timeWindows)
    datToPlot = pd.DataFrame(
        {
...
def fig_average_predLoss_vs_euclError(
    predLossTicks, euclError, timeWindows=[36, 108, 252, 504], dirSave=None, suffix=""
):
    ...
    for iw in range(len(timeWindows)):
        ...
        ax.fill_between(
            predLossTicks,
            euclErrorAv[iw] - euclErrorSEM[iw],
            euclErrorAv[iw] + euclErrorSEM[iw],
            color=cm(12 + iw),
            alpha=0.2,
        )
